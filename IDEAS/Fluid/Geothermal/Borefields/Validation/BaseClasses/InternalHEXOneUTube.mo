...
public
  // Actuele mediumtoestanden van de twee volumes
  Medium.ThermodynamicState sta1 = vol1.medium.state;
  Medium.ThermodynamicState sta2 = vol2.medium.state;

  // Dynamische mediumeigenschappen op runtime
  Real muMed1_dyn = Medium.dynamicViscosity(sta1);
  Real muMed2_dyn = Medium.dynamicViscosity(sta2);

  Real kMed1_dyn  = Medium.thermalConductivity(sta1);
  Real kMed2_dyn  = Medium.thermalConductivity(sta2);

  Real cpMed1_dyn = Medium.specificHeatCapacityCp(sta1);
  Real cpMed2_dyn = Medium.specificHeatCapacityCp(sta2);
